...
.sidebar-tabs button[data-tooltip]:hover::after,
.timeline-header h3[data-tooltip]:hover::after {
...
.sidebar-tabs button, .timeline-header h3 {
  position: relative;
}
.file-list {
  flex: 1;
...
.timeline-clip:hover .clip-actions { display: flex; }
...
.timeline-clip .clip-actions button:hover { background: rgba(255,255,255,0.3); }
.timeline-clip.dragging { opacity: 0.5; }
.timeline-track.drag-over { background: rgba(11,132,243,0.1); }
.timeline-label .label-actions { display: none; margin-left: auto; }
.timeline-label:hover .label-actions { display: flex; gap: 2px; }
.timeline-label .label-actions button {
  background: none;
  border: none;
  font-size: 11px;
  cursor: pointer;
...
.timeline-label .label-actions button:hover { color: var(--red); }
...
<div class="editor" id="app">
  ...
  <div class="main">
    ...
    <div class="sidebar-left" id="sidebarLeft">
      <div class="sidebar-tabs">
        <button class="active" onclick="switchTab(this,'media')" data-tooltip-key="tab_media">Media</button>
        <button onclick="switchTab(this,'output')" data-tooltip-key="tab_output">Output</button>
      </div>
    ...
    <div class="sidebar-right" id="sidebarRight">
      <div class="sidebar-tabs">
        <button class="active" onclick="switchRightTab(this,'props')" data-tooltip-key="tab_properties">Properties</button>
        <button onclick="switchRightTab(this,'skills')" data-tooltip-key="tab_skills">Skills</button>
      </div>
      <div id="propsPanel">
  ...
  <div class="timeline-area" id="timelineArea">
    <div class="timeline-header">
      <h3 data-tooltip-key="tab_timeline">Timeline</h3>
      <select id="projectSelect" onchange="loadProject(this.value)">
        <option value="">-- Select Project --</option>
      </select>
      <span class="spacer"></span>
      <button onclick="newProject()" style="background:var(--bg-input);border:1px solid var(--border);color:var(--text);padding:2px 8px;border-radius:4px;font-size:11px;cursor:pointer">+ New</button>
      <button onclick="saveProject()" style="background:var(--bg-input);border:1px solid var(--border);color:var(--text);padding:2px 8px;border-radius:4px;font-size:11px;cursor:pointer">Save</button>
    </div>
...
const I18N = {
  en: {
    tab_media: "Media files — drag & drop or upload source videos, audio, subtitles here",
    tab_output: "Rendered output — files generated by skill/project runs appear here",
    tab_properties: "File properties — metadata and ffprobe info for the selected file",
    tab_skills: "Available skills — ffmpeg operations you can run on media files",
    tab_timeline: "Timeline — arrange and edit project pipeline steps, drag to reorder",
  },
  ja: {
    tab_media: "素材ファイル — 動画・音声・字幕をドラッグ＆ドロップまたはアップロード",
    tab_output: "出力ファイル — スキル/プロジェクト実行で生成されたファイル",
    tab_properties: "ファイル情報 — 選択ファイルのメタデータとffprobe情報",
    tab_skills: "スキル一覧 — メディアファイルに使えるffmpeg操作",
    tab_timeline: "タイムライン — プロジェクトのステップを並べ替え・編集",
  },
};
const LANG = (navigator.language || 'en').startsWith('ja') ? 'ja' : 'en';
function i18n(key) { return (I18N[LANG] || I18N.en)[key] || (I18N.en)[key] || key; }
...
function addToTimeline(list, index) {
  const file = (list === 'media' ? state.media : state.output)[index];
  if (!file) return;
  const skill = file.type === 'audio' ? 'mix-audio' : file.type === 'subtitle' ? 'subtitles' : 'reformat';
  const params = {};
  if (file.type === 'audio') {
    params.audio = file.relpath;
    params.video = '${prev.output}';
    params.replace = true;
  } else if (file.type === 'subtitle') {
    params.sub = file.relpath;
    params.video = '${prev.output}';
  } else {
    params.video = file.relpath;
  }
  addStep(skill, params);
}
...
function renderProjectSelect() {
  ...
  const cur = state.currentProject?.name || '';
  el.innerHTML = '<option value="">-- Select Project --</option>' +
    state.projects.map(p => `<option value="${esc(p.name)}" ${p.name===cur?'selected':''}>${esc(p.name)} (${p.steps?.length || 0} steps)</option>`).join('');
}
...
function loadProject(name) {
  ...
  if (!proj) return;
  state.currentProject = JSON.parse(JSON.stringify(proj)); // deep copy for editing
  state.dirty = false;
  renderTimeline();
}
...
function clipType(skill) {
  if (skill === 'probe') return 'effect';
  if (skill.includes('audio') || skill.includes('mix')) return 'audio';
  if (skill.includes('subtitle')) return 'subtitle';
  if (skill === 'reformat') return 'video';
  return 'video';
}

function clipSummary(step) {
  const p = step.params || {};
  const parts = [step.skill];
  if (p.video) parts.push(p.video.split('/').pop().replace('${prev.output}','← prev'));
  if (p.audio) parts.push(p.audio.split('/').pop());
  if (p.sub || p.srt || p.ass) parts.push((p.sub||p.srt||p.ass).split('/').pop());
  if (p.in != null || p.out != null) parts.push(`${p.in||0}s–${p.out||'end'}`);
  if (p.replace) parts.push('replace');
  if (p.volume != null) parts.push(`vol:${p.volume}`);
  if (p.scale) parts.push(`${p.scale.w}x${p.scale.h}`);
  if (p.crop) parts.push('crop');
  return parts.join(' · ');
}
...
function renderTimeline() {
  ...
  const totalDuration = Math.max(steps.length * 30, 60);
  const totalWidth = totalDuration * PX_PER_SEC;

  // Labels with delete button
  const labels = document.getElementById('timelineLabels');
  labels.innerHTML = steps.map((s, i) => `
    <div class="timeline-label" draggable="true"
         ondragstart="dragStep(event,${i})" ondragover="event.preventDefault()"
         ondrop="dropStep(event,${i})">
      ${i+1}. ${s.skill}
      <span class="label-actions">
        <button onclick="removeStep(${i})" title="Remove">×</button>
      </span>
    </div>
  `).join('');
  ...
  steps.forEach((step, i) => {
    const left = i * 30 * PX_PER_SEC;
    const width = 28 * PX_PER_SEC;
    const ct = clipType(step.skill);
    const summary = clipSummary(step);
    html += `
      <div class="timeline-track" ondragover="event.preventDefault();this.classList.add('drag-over')"
           ondragleave="this.classList.remove('drag-over')" ondrop="dropStep(event,${i});this.classList.remove('drag-over')">
        <div class="timeline-clip ${ct}" style="left:${left}px;width:${width}px"
             title="${esc(summary)}" draggable="true"
             ondragstart="dragStep(event,${i})" onclick="editStep(${i})">
          ${step.skill}
          <span class="clip-actions">
            <button onclick="event.stopPropagation();removeStep(${i})">×</button>
          </span>
        </div>
      </div>
    `;
  ...
  drawRuler(totalDuration, PX_PER_SEC);
}
...
// ── Drag & Drop Reorder ──
let _dragIndex = -1;
function dragStep(e, i) { _dragIndex = i; e.dataTransfer.effectAllowed = 'move'; }
function dropStep(e, targetIndex) {
  e.preventDefault();
  if (_dragIndex < 0 || _dragIndex === targetIndex) return;
  const proj = state.currentProject;
  if (!proj) return;
  const steps = proj.steps;
  const [moved] = steps.splice(_dragIndex, 1);
  steps.splice(targetIndex, 0, moved);
  _dragIndex = -1;
  state.dirty = true;
  renderTimeline();
  toast('Step moved — Save to persist');
}
...
function removeStep(i) {
  const proj = state.currentProject;
  if (!proj) return;
  const step = proj.steps[i];
  if (!confirm(`Remove step ${i+1} (${step.skill})?`)) return;
  proj.steps.splice(i, 1);
  state.dirty = true;
  renderTimeline();
  toast('Step removed — Save to persist');
}
...
function editStep(i) {
  ...
  if (!proj) return;
  const step = proj.steps[i];
  const json = prompt('Edit step params (JSON):', JSON.stringify(step.params, null, 2));
  if (json === null) return;
  try {
    step.params = JSON.parse(json);
    state.dirty = true;
    renderTimeline();
    toast('Step updated — Save to persist');
  } catch (e) {
    toast('Invalid JSON');
  }
}

function addStep(skill, params) {
  const proj = state.currentProject;
  if (!proj) { toast('Select or create a project first'); return; }
  proj.steps.push({ skill, params });
  state.dirty = true;
  renderTimeline();
  toast(`Added ${skill} step — Save to persist`);
}
...
async function saveProject() {
  ...
  const res = await api('/api/project/save', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: proj.name, steps: proj.steps }),
  });
  if (res.status === 'success') {
    state.dirty = false;
    toast(`Saved ${proj.name}`);
...
document.querySelectorAll('[data-tooltip-key]').forEach(el => {
  el.setAttribute('data-tooltip', i18n(el.getAttribute('data-tooltip-key')));
});
